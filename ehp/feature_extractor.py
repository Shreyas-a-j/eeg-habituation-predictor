...
class EEGFeatureExtractor:
    ...
    def bandpower(self, signal):
        import numpy as np
        fft = np.abs(np.fft.rfft(signal))

        return {
            "delta": np.mean(fft[0:4]),
            "theta": np.mean(fft[4:8]),
            "alpha": np.mean(fft[8:13]),
            "beta": np.mean(fft[13:30])
        }
    ...
    def extract_features(self, signal):
        ...
        # Compute delta, theta, alpha, beta powers
        delta = np.mean(pxx[(freqs >= 0.5) & (freqs < 4)])
        ...
        bp = self.bandpower(signal)  # optional, keep if needed
        ...
        # Merge bandpower dictionary if it exists
        if bp:
            features.update(bp)
